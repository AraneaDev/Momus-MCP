...
const sp = (file: string, sl: number, sc = 1, el = sl, ec = 2): SourceSpan => ({ file, startLine: sl, startCol: sc, endLine: el, endCol: ec });

function testModule(over: Partial<ModuleIR> = {}): ModuleIR {
  return {
    path: FILE, language: 'typescript', kind: 'test', framework: 'vitest',
    imports: [{ specifier: '../src/ledger', names: ['LedgerService'] }],
    symbols: [], exports: [], mocks: [], assertions: [], functions: [],
    comments: [], diagnostics: [], hash: 'x',
    ...over,
  };
}

const expr = (over: Partial<ExprIR> = {}): ExprIR => ({
  kind: 'call', text: 'x', mockRefs: [], provenance: 'unknown', constant: false, ...over,
});

const assertion = (over: Partial<AssertionIR> = {}): AssertionIR => ({
  id: 'a1', span: sp(FILE, 10), api: 'toBe', operands: [], fnId: 'f1', ...over,
});

const mock = (over: Partial<MockIR> = {}): MockIR => ({
  id: 'm1', span: sp(FILE, 5), framework: 'vitest', pattern: 'vi.fn',
  stubbedMembers: [], configuredValues: [], invocationSites: [], isAutomock: false, ...over,
});
...
describe('TAUT-002 mock echo', () => {
  it('flags literal matching a configured mock value', () => {
    const m = testModule({ assertions: [assertion({
      operands: [
...
describe('TAUT-003 constant tautology', () => {
  it('flags two literal operands', () => {
    const m = testModule({ assertions: [assertion({
      api: 'toEqual',
      operands: [expr({ text: '1', provenance: 'literal', constant: true }), expr({ text: '1', provenance: 'literal', constant: true })],
    })] });
    expect(runRules(rulesOf('TAUT-003'), ctx(m))).toHaveLength(1);
  });
...
describe('TAUT-004 mock-only assertion', () => {
  it('flags mock-only assertion in a test with no production calls', () => {
    const m = testModule({
      functions: [{ id: 'f1', span: sp(FILE, 1), hasProductionCalls: false, productionCallCount: 0, assertionCount: 1 }],
...
describe('TAUT-005 zero-reach stub', () => {
  it('flags configured mocks that are never invoked or asserted', () => {
    const m = testModule({
      mocks: [mock({ id: 'm1', configuredValues: [{ span: sp(FILE, 5), api: 'mockReturnValue', once: false, assignable: 'unknown' }] })],
    });
    expect(runRules(rulesOf('TAUT-005'), ctx(m))).toHaveLength(1);
  });
  it('ignores invoked or asserted mocks', () => {
    const invoked = testModule({
      mocks: [mock({ id: 'm1', configuredValues: [{ span: sp(FILE, 5), api: 'mockReturnValue', once: false, assignable: 'unknown' }], invocationSites: [sp(FILE, 9)] })],
    });
...
describe('TAUT-006 unconfigured spy assert', () => {
  it('flags toHaveBeenCalled on an unconfigured, unreached spy', () => {
    const m = testModule({
      mocks: [mock({ id: 's1', pattern: 'vi.spyOn', stubbedMembers: [{ name: 'm', span: sp(FILE, 5), api: 'spyOn', returnValues: [] }] })],
      assertions: [assertion({ api: 'toHaveBeenCalled', operands: [expr({ mockRefs: ['s1'], provenance: 'mock-call' })] })],
    });
  ...
  it('stays quiet for configured or reached spies', () => {
    const configured = testModule({
      mocks: [mock({ id: 's1', pattern: 'vi.spyOn', configuredValues: [{ span: sp(FILE, 5), api: 'mockReturnValue', once: false, assignable: 'unknown' }] })],
...
describe('DRIFT-001 missing member', () => {
  const prod: ModuleIR = {
    path: PROD, language: 'typescript', kind: 'production',
    imports: [], symbols: [{
      id: `${PROD}#LedgerService`, name: 'LedgerService', kind: 'class', span: sp(PROD, 1),
      members: [{ id: `${PROD}#LedgerService.totalFor`, name: 'totalFor', kind: 'method', span: sp(PROD, 3), members: [] }],
      extendsIds: [], implementsIds: [],
    }], exports: ['LedgerService'], mocks: [], assertions: [], functions: [],
    comments: [], diagnostics: [], hash: 'y',
  };
  const index = new SymbolIndex([prod]);

  it('flags stubs for members missing from the production class', () => {
    const m = testModule({ mocks: [mock({
      id: 'm1', pattern: 'vi.spyOn',
      target: { kind: 'instance-member', symbolId: `${PROD}#LedgerService`, memberName: 'totalForX', span: sp(FILE, 5) },
  ...
  it('accepts existing members', () => {
    const m = testModule({ mocks: [mock({
      id: 'm1', pattern: 'vi.spyOn',
      target: { kind: 'instance-member', symbolId: `${PROD}#LedgerService`, memberName: 'totalFor', span: sp(FILE, 5) },
...
describe('DRIFT-005 missing export', () => {
  const prod: ModuleIR = {
    path: PROD, language: 'typescript', kind: 'production',
    imports: [], symbols: [{ id: `${PROD}#Db`, name: 'Db', kind: 'class', span: sp(PROD, 1), members: [], extendsIds: [], implementsIds: [] }],
    exports: ['Db'], mocks: [], assertions: [], functions: [], comments: [], diagnostics: [], hash: 'y',
  };
  const index = new SymbolIndex([prod]);

  it('flags factory keys not exported by the target module', () => {
    const m = testModule({ mocks: [mock({
      id: 'm1', pattern: 'vi.mock',
...
describe('rule engine', () => {
  ...
  it('wraps rule crashes as internal errors without aborting', () => {
    ...
    const badRule = {
      id: 'TAUT-001' as const, name: 'bad', defaultSeverity: 'error' as const, description: '',
      appliesTo: () => true,
