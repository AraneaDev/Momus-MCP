...
for (const f of ['src/services/db.ts', 'src/services/ledger.ts']) {
  ...
  console.log(f, 'symbols:', m.symbols.map((s) => `${s.id} [${s.kind}] members=${s.members.map((x) => x.name).join(',')}`));
}
// what does the checker resolve for the spyOn targets?
const src = readFileSync(root + '/tests/ledger.test.ts', 'utf8');
const tm = parser.parseModule(root + '/tests/ledger.test.ts', src, { config: {} as never, resolveImport: (s) => parser.resolveImport(s, root + '/tests/ledger.test.ts') });
for (const m of tm.mocks) {
  console.log('mock:', m.pattern, 'target:', JSON.stringify(m.target), 'stubs:', m.stubbedMembers.map((s) => s.name).join(','));
}
