...
> blame, and harsh criticism** — the one god whose entire job was to find fault, and who was
> finally thrown off Olympus for doing it too well. His name literally translates to
> **"blame"** or **"censure"**.

Momus-MCP is a **local-first, deterministic, read-only** MCP server and CLI that audited test
...
tautological assertions, mock-contract drift, and mock-hygiene problems, not because the code
works. The name is doubly apt: Momus is the god of *mockery*, and mock objects are exactly what
this tool scrutinizes.

> **Status:** pre-release. Momus-MCP is **not yet published to npm** — the publish step in the
> release workflow is deliberately dormant (credential-blocked). The source is public; install
> from source (see [Quick Start](#quick-start)). Any `npx @momus/*` command you find elsewhere
> will not resolve until the packages are published.
