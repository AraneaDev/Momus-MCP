...
| DRIFT-005 | `missing-export` | error | `vi.mock('mod', factory)` / `jest.mock` factory keys (top-level) reference exports that don't exist on the resolved module; or the real module exports a name the factory omits while the test imports that name from the mocked module. Exports are matched against the module's **full named-export list** — including `const`/`type`/`enum` declarations and barrel re-exports (`export { X } from '…'` / `export * as ns from '…'`), not just class/function symbols. In Python, `patch('mod.attr')` / `patch.object` targets are checked the same way: the final dotted segment must be an attribute of the resolved module, where the module's attributes include its top-level import bindings. |
